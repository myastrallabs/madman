defmodule Madman.TradeRobot.Leader do
  @moduledoc false
  ...
  alias Madman.TradeRobot.Trader
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        ...
        if settings.status == "shutdown" and traders == [] do
          Madman.TradeRobot.stop_trading(state.symbol)
        end
  ...
  defp fetch_symbol_settings(symbol) do
    ...
    settings = Repo.get_by!(TradingSetting, symbol: symbol)
  ...
  defp start_new_trader(%Trader.State{} = state) do
    {:ok, pid} =
      DynamicSupervisor.start_child(
        :"Madman.TradeRobot.DynamicTraderSupervisor-#{state.symbol}",
        {Madman.TradeRobot.Trader, state}
      )
