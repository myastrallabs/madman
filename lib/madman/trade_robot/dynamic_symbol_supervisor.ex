defmodule Madman.TradeRobot.DynamicSymbolSupervisor do
  ...
  alias Madman.TradeRobot.SymbolSupervisor
  ...
  def shutdown_worker(symbol) when is_binary(symbol) do
    ...
    Madman.TradeRobot.Leader.notify(:settings_updated, settings)
    {:ok, settings}
  end

  defp update_status(symbol, status)
       when is_binary(symbol) and is_binary(status) do
    Repo.get_by(TradingSetting, symbol: symbol)
